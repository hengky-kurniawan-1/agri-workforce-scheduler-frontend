...
export function AgronomistRouteList({
	routes,
	visibleIds,
	onToggleAgronomist,
	onSetAllVisible,
	...
	return (
		<section className="rounded-lg border border-soil-200 bg-white p-4">
			<div className="flex items-start justify-between gap-2">
				<h2 className="text-xs font-semibold uppercase tracking-wide text-soil-500">
					Route by agronomist
				</h2>
				...
				{routes.map((r) => {
					...
					return (
						...
						>
							...
							{r.stops.length === 0 ? (
								<p className="mt-2 pl-6 text-soil-600">
									No stops (missing field data or no assignments).
								</p>
							) : (
								<ol className="mt-2 list-decimal space-y-1 pl-10 text-soil-700">
									{r.stops.map((s) => (
										<li key={`${r.agronomistId}-${s.kind}-${s.fieldId}-${s.taskId ?? 'home'}`}>
											{s.kind === 'home' ? (
												<>
